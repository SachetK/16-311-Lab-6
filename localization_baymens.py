...
def bayesian_localization(map, sensor_reading, motion_step, belief, sensor_model, motion_model):
    ...
    for i in range(num_sectors):
        ...
        match_prob = sensor_model[expected_reading == sensor_reading]  # Get probability from sensor model
...
def main():
    ...
    belief = np.ones(len(map)) / len(map)

    # Define Sensor Model (P(sensor reading | true state))
    sensor_model = {True: 0.9, False: 0.1}  # 90% accuracy
    ...
    sensor_reading = 10 #vl53.distance  # Assume sensor detects a wall

    # Motion step (robot moves one sector)
    motion_step = 1

    # Update belief
    new_belief = bayesian_localization(map, sensor_reading, motion_step, belief, sensor_model, motion_model)

    # Output most probable location
    most_likely_sector = np.argmax(new_belief)
    print(f"Most likely sector: {most_likely_sector}, Probability: {new_belief[most_likely_sector]:.2f}")
    iterations = 5
    for i in range(iterations):
        sensor_reading = np.random.choice([0, 1], p=[0.5, 0.5])  # Simulated sensor reading
        motion_step = 1  # Robot moves one step

        # Update belief
        belief = bayesian_localization(map, sensor_reading, motion_step, belief, sensor_model, motion_model)
